...
function removeDup(arr: any[]): any[] {
  var result: any[] = [];
  var obj: any = {};
...
export function getDigitalCount(n: number) {
  const numStr = n.toString().trimEnd();
  const pointIndex = numStr.indexOf('.');
  return numStr.length - pointIndex - 1;
}

export function getRandom(min: number = 0, max: number = 100) {
  let root = Math.random();
  return Number(Math.floor(root * (max - min) + min));
}

export function ifSuccess(rate: number): boolean {
  // 将传入的百分制数字转换为概率值（0 到 1 之间）
  const probability = rate / 100;

  // 生成一个随机数（0 到 1 之间）
  const random = Math.random();

  // 判断随机数是否小于等于概率值，如果是，则返回 true，否则返回 false
  return random <= probability;
}

export function getResult(arr: number[]) {
  let sum: number = 0;
  for (var i = 0; i < arr.length; i++) {
    sum += arr[i]                                     //获取总数
  }
  for (var i = 0; i < arr.length; i++) {
    var random = Math.random() * sum;       //获取 0-总数 之间的一个随随机整数
    if (random < arr[i]) {
...
export function getRandomName() {
  var firstNames = new Array(
    ...
    "太叔", "申屠", "公孙", "仲孙", "轩辕", "令狐", "宇文", "长孙", "慕容", 
    "闾丘", "司徒", "司空",
    "乐正", "公良", "拓拔", "谷梁", "段干", "百里", "东郭", "南门", "呼延",
    "羊舌", "梁丘", "左丘", "东门", "西门", "南宫"
  );
  ...
  lastNames = removeDup(lastNames);

  var firstLength = firstNames.length;
  var lastLength = lastNames.length;

  var i = +(Math.random() * firstLength).toFixed(0);
  var j = +(Math.random() * lastLength).toFixed(0);
...
export function getArraryFromRandom( root: number, length: number = 8): number[] {
  let leftRoot = root;
  let list: number[] = [];
